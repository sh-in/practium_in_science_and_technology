...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(13):
            ...
            h = self.relu1(h)
            h = self.pool1(h)
            # print(h.shape) # torch.Size([128, 64, 15, 15])
            hs.append(h)

        h = hs[0].cpu().detach().numpy()
        for i in range(1, 13):
            h = np.concatenate((h, hs[i].cpu().detach().numpy()), axis=1)
...
plt.savefig(output_path+"loss" + str(120) +".png")
...
plt.xlabel("epoch")
plt.ylabel("accuracy")
plt.savefig(output_path+"acc" + str(120) + ".png")
plt.clf()
plt.close()
...
submissions.to_csv(output_path+"wavelets_submissions" + str(120) + ".csv", index=False, header=True)
