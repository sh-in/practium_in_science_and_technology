...
if os.path.exists(output_path + "result/transformer_comp/feature_comp") == False:
    os.makedirs(output_path + "result/transformer_comp/feature_comp")
output_path = output_path + "result/transformer_comp/feature_comp/"
...
# Merge data sets
train_sets = [Train1, Train2, Train3, Train4]
...
test_sets = [Test1, Test2, Test3, Test4]
for i in range(4):
...
print(l)
for cs in range(l):
    columns_list.append(features)
    traintmp = train.reshape(-1, 60, train.shape[-1])
    testtmp = test.reshape(-1, 60, test.shape[-1])

    # train validation split
    train_x, val_x, train_y, val_y = train_test_split(traintmp, labels, test_size=0.2, shuffle=False)
    ...
    # Create test tensor
    ttest = torch.from_numpy(testtmp)
    ...
    print("Model: {}, features{}".format(44+cs, features))
    ...
    plt.savefig(output_path+"loss" + str(44+cs) + ".png")
    ...
    plt.xlabel("epoch")
    plt.ylabel("accuracy")
    plt.savefig(output_path+"acc" + str(44+cs) + ".png")
    ...
    submissions.to_csv(output_path + "transformer_submissions" + str(44+cs) +  ".csv", index=False, header=True)
