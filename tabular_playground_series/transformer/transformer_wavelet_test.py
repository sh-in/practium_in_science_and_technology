...
Train = pd.read_csv("../../../../output/Tabular_Playground_Series_Apr_2022/new_dataset/wavelets/newTrainDataset.csv", dtype=np.float)
...
Test = pd.read_csv("../../../../output/Tabular_Playground_Series_Apr_2022/new_dataset/wavelets/newTestDataset.csv", dtype=np.float32)
...
print(Test.head())
print("Train.shape", Train.shape) # df_train.shape (25968, 13)
print("Test.shape", Test.shape)# df_test.shape (12218, 13)
...
plt.savefig(output_path+"loss" + str(114) + ".png")
...
plt.xlabel("epoch")
plt.ylabel("accuracy")
plt.savefig(output_path+"acc" + str(114) + ".png")
...
submissions.to_csv(output_path + "transformer_submissions" + str(114) +  ".csv", index=False, header=True)
