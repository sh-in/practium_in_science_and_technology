...
if os.path.exists(output_path + "result/transformer_comp/remove_features") == False:
    os.makedirs(output_path + "result/transformer_comp/remove_features")
output_path = output_path + "result/transformer_comp/remove_features/"
...
# Dataset with lowpass. Input 39
Train5 = pd.read_csv("../../../../output/Tabular_Playground_Series_Apr_2022/new_dataset/fft/newTrainDataset.csv", dtype=np.float32)
...
for i in range(5):
    Train = pd.merge(Train, train_sets[i])
...
# Dataset with warp. input 26
Test5 = pd.read_csv("../../../../output/Tabular_Playground_Series_Apr_2022/new_dataset/fft/newTestDataset.csv", dtype=np.float32)

# Merge data sets
test_sets = [Test1, Test2, Test3, Test4, Test5]
for i in range(5):
    Test = pd.merge(Test, test_sets[i])
...
train = Train.drop(["sequence", "subject", "step"], axis=1)
train = train.drop(train.columns[[14, 17, 19, 20, 22, 24, 26, 27, 28, 30, 31, 32, 33, 36, 37, 39, 42, 44, 45, 47, 49, 50, 52, 54, 57, 58, 60, 63, 65, 68, 69, 71, 72, 75, 76, 77, 80, 81]], axis=1)
...
test = Test.drop(["sequence", "subject", "step"], axis=1)
test = test.drop(test.columns[[14, 17, 19, 20, 22, 24, 26, 27, 28, 30, 31, 32, 33, 36, 37, 39, 42, 44, 45, 47, 49, 50, 52, 54, 57, 58, 60, 63, 65, 68, 69, 71, 72, 75, 76, 77, 80, 81]], axis=1).values
...
print("Model: {}, features{}".format(113, columns_list))
...
plt.savefig(output_path+"loss" + str(116) + ".png")
...
plt.xlabel("epoch")
plt.ylabel("accuracy")
plt.savefig(output_path+"acc" + str(116) + ".png")
...
submissions.to_csv(output_path + "transformer_submissions" + str(116) +  ".csv", index=False, header=True)
...
# c=0
# for i in score_list:
#     print("{:5}{:8}{:14.5}{:13.5}{:12.5}{:11.5}".format(c+44, i[0], i[1], i[2], i[3], i[4]))
#     c+=1
# f = open(output_path + "score_list.csv", "w", newline="")
# writer = csv.writer(f)
# writer.writerows(score_list)
# f.close()

# Output columns lists
# need to search how to convert list to csv in pandas
f = open(output_path + "columns_lists.csv", "w", newline="")
writer = csv.writer(f)
