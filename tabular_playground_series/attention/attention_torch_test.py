# Change decoder to (linear, relu, dropout, sigmoid) from base attention.
...
class LSTM_Attention(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers, bidirectional=True):
        ...
        # Encoder
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers+1, batch_first=True, dropout=0.5, bidirectional=bidirectional)
        ...
        # Decoder
        self.out1 = nn.Linear(hidden_size, hidden_size, bias=True)
        self.dropout = nn.Dropout(0.5)
        self.out2 = nn.Linear(hidden_size, output_size, bias=True)
...
if os.path.exists(output_path + "attention/torch/test") == False:
    os.makedirs(output_path + "attention/torch/test")
output_path = output_path + "attention/torch/test/"
