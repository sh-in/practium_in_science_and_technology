...
class LSTM_Attention(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers, bidirectional=True):
        ...
        # Encoder
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, dropout=0.5, bidirectional=bidirectional)
        ...
        # Decoder
        self.decoder = nn.LSTM(hidden_size, hidden_size, num_layers, batch_first=True, bidirectional=bidirectional)
        self.out = nn.Linear(hidden_size*2, output_size, bias=True)
    ...
    def forward(self, x):
        ...
        # Decoder
        # I need to change the shape of the attn_hidden 2d to 3d.
        attn_reshape = torch.reshape(attn_hidden, (out.size(dim=0), 1, self.hidden_size))
        # print("attn_reshape.shape", attn_reshape.shape) # attn_reshape.shape torch.Size([32, 1, 128])
        y, (hd, cd) = self.decoder(attn_reshape, (hn, cn))
        y = torch.cat([hd[0], hd[1]], dim=1)
        y = self.out(y)
...
if os.path.exists(output_path + "attention/torch/d_lstm") == False:
    os.makedirs(output_path + "attention/torch/d_lstm")
output_path = output_path + "attention/torch/d_lstm/"
