# Change the way to calculate attention weights.
...
class LSTM_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # e = torch.zeros(out.size(0), out.size(1), out.size(0))
        # print("e.shape", e.shape) # torch.Size([32, 60, 32])
        # out_permute = out.permute(2, 1, 0)
        # print("out_permute.shape", out_permute.shape) # torch.Size([256, 60, 32])
        # print("out_permute[:, 0, :].shape", out_permute[:, 0, :].shape) # torch.Size([256, 32])
        for i in range(out.size(1)):
            e[:, i, :] = (biout*out[:, i, :])/math.sqrt(self.hidden_size)
        attention_weights = F.softmax(e, dim=1)
        # print("attention_weights.shape", attention_weights.shape) # torch.Size([32, 60, 32])
        ...
        # Decoder
        y = self.out(attn_hidden)
        # print("y.shape", y.shape) # torch.Size([32, 2])
        # sig = nn.Sigmoid()
        # y = sig(self.dropout(y))
...
if os.path.exists(output_path + "attention/torch/scaled_dot") == False:
    os.makedirs(output_path + "attention/torch/scaled_dot")
output_path = output_path + "attention/torch/scaled_dot/"
