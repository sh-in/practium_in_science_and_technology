...
preds = torch.from_numpy(np.concatenate(preds, 0).flatten())
train_preds = pd.DataFrame({"bi_lstm": preds})
print("train_preds", train_preds)
train_preds.to_csv("../../../../output/Tabular_Playground_Series_Apr_2022/result/transformer_comp/ensemble/bi_lstm_train.csv", index=False, header=True)
...
with torch.no_grad():
    ...
    for x in test_loader:
        ...
        # preds += pred
preds = torch.from_numpy(np.concatenate(preds, 0).flatten())
test_preds = pd.DataFrame({"bi_lstm": preds})
print("test_preds", test_preds)
test_preds.to_csv("../../../../output/Tabular_Playground_Series_Apr_2022/result/transformer_comp/ensemble/bi_lstm_test.csv", index=False, header=True)
preds = torch.round(preds).to(torch.int64)
