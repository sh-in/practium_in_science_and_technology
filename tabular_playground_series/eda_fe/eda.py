...
# stats_df = stats_df.drop("state", axis=1)
...
# Create new dataset
# Train = pd.read_csv(ds_path + "train.csv", dtype=np.float32)
# Train["sensor_04_mean"] = Train.groupby("sequence")["sensor_04"].transform("mean")
# Train["sensor_02_std"] = Train.groupby("sequence")["sensor_02"].transform("std")
# kurtosis = []
# for i in range(Train["sequence"].nunique()):
#     kurt = Train[Train.sequence==i]["sensor_04"].kurt()
#     for j in range(60):
#         kurtosis.append(kurt)
# Train["sensor_04_kurtosis"] = kurtosis
# kurtosis = []
# for i in range(Train["sequence"].nunique()):
#     kurt = Train[Train.sequence==i]["sensor_10"].kurt()
#     for j in range(60):
#         kurtosis.append(kurt)
# Train["sensor_10_kurtosis"] = kurtosis

# Train.to_csv(output_path+"newTrain.csv", index=False, header=True)

# Test = pd.read_csv(ds_path + "test.csv", dtype=np.float32)
# Test["sensor_04_mean"] = Test.groupby("sequence")["sensor_04"].transform("mean")
# Test["sensor_02_std"] = Test.groupby("sequence")["sensor_02"].transform("std")
# kurtosis = []
# for i in range(Test["sequence"].nunique()):
#     kurt = Test[Test.sequence==i]["sensor_04"].kurt()
#     for j in range(60):
#         kurtosis.append(kurt)
# Test["sensor_04_kurtosis"] = kurtosis
# kurtosis = []
# for i in range(Test["sequence"].nunique()):
#     kurt = Test[Test.sequence==i]["sensor_10"].kurt()
#     for j in range(60):
#         kurtosis.append(kurt)
# Test["sensor_10_kurtosis"] = kurtosis

# Test.to_csv(output_path+"newTest.csv", index=False, header=True)

# Add lag to dataset
train = pd.read_csv(output_path + "newTrain.csv", dtype=np.float32)
for sensor in range(13):
    ...
    train[f"{sensor_name}" + "_diff1"] = train[f"{sensor_name}"] - train[f"{sensor_name}" + "_lag1"]
train.to_csv(output_path + "addTrainLag.csv", index=False, header=True)

test = pd.read_csv(output_path + "newTest.csv", dtype=np.float32)
for sensor in range(13):
    sensor_name = f"sensor_{sensor:02d}"
    test[f"{sensor_name}" + "_lag1"] = test.groupby("sequence")[f"{sensor_name}"].shift(1)
    test[f"{sensor_name}" + "_lag1"].fillna(0, inplace=True)
    test[f"{sensor_name}" + "_diff1"] = test[f"{sensor_name}"] - test[f"{sensor_name}" + "_lag1"]
test.to_csv(output_path + "addTestLag.csv", index=False, header=True)