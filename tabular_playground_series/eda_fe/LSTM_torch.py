...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # y = y_lstm[:, -1, :]
        y = self.fc(y_lstm[:, -1, :]) # Forward LSTM final result to fully connection.
        return y


# Set output path
output_path = "../../../../output/Tabular_Playground_Series_Apr_2022/"
if os.path.exists(output_path + "eda_fe/lstm") == False:
    os.makedirs(output_path + "eda_fe/lstm")
output_path = output_path + "eda_fe/lstm/"
...
submissions.to_csv(output_path + "lstm_torch_submissions.csv", index=False, header=True)
